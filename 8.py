...
conn = sqlite3.connect("industrial_fan.db", check_same_thread=False)
cursor = conn.cursor()

# Create the Industrial_Fan table if it doesn't exist
cursor.execute("""
CREATE TABLE IF NOT EXISTS Industrial_Fan (
...
cursor.execute("PRAGMA table_info(Industrial_Fan)")
columns = [column[1] for column in cursor.fetchall()]
if "Failure_Type" not in columns:
    cursor.execute("""
    CREATE TABLE IF NOT EXISTS Industrial_Fan_New (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        Timestamp TEXT NOT NULL,
        Vibration INTEGER NOT NULL,
        RPM INTEGER NOT NULL,
    ...
    """)
    cursor.execute("""
    INSERT INTO Industrial_Fan_New (id, Timestamp, Vibration, RPM, Temp, Status)
    SELECT id, Timestamp, Vibration, RPM, Temp, Status FROM Industrial_Fan
    """)
    cursor.execute("DROP TABLE Industrial_Fan")
    cursor.execute("ALTER TABLE Industrial_Fan_New RENAME TO Industrial_Fan")
    conn.commit()

# Limits for Industrial Fan
limits = {
...
def insert_to_db(timestamp, vibration, rpm, temp, status, failure_type):
    cursor.execute("""
        INSERT INTO Industrial_Fan (Timestamp, Vibration, RPM, Temp, Status, Failure_Type)
        VALUES (?, ?, ?, ?, ?, ?)
...
def get_data():
    cursor.execute("SELECT * FROM Industrial_Fan")
...
def reset_monitoring():
    ...
    cursor.execute("DELETE FROM Industrial_Fan")
    conn.commit()
